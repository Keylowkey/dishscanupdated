...
export function normalizePem(raw) {
  ...
  // Accept the whole PEM base64-encoded as one line. Multi-line values get
  // mangled by env-var inputs in too many ways; a single base64 blob can't be.
  if (!s.includes('-----BEGIN')) {
    try {
      const decoded = Buffer.from(s.replace(/\s+/g, ''), 'base64').toString('utf8');
      if (decoded.includes('-----BEGIN')) s = decoded.replace(/\r/g, '').trim();
    } catch (e) { /* not base64 — fall through */ }
  }
