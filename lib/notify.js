...
export async function notifyAll(cfg, rows) {
  ...
  try {
    ...
    // platform decides the gateway. An APNs token sent to FCM (or the reverse)
    // is simply rejected, so the two must be kept apart.
    const tr = await fetch(
      `${SUPABASE_URL}/rest/v1/device_tokens?select=token,user_id,platform&user_id=in.${inList}`,
      { headers });
    const tokenRows = tr.ok ? await tr.json() : [];
    if (tokenRows.length) {
      const byUser = {};
      tokenRows.forEach(t => {
        const u = (byUser[t.user_id] = byUser[t.user_id] || { ios: [], android: [] });
        (t.platform === 'android' ? u.android : u.ios).push(t.token);
      });

      const dead = [];
      for (const row of list) {
        const devices = byUser[row.recipient_id];
        if (!devices) continue;
        ...
        // One person may carry both an iPhone and an Android phone.
        if (devices.ios.length) {
          const out = await sendPush(devices.ios, payload);
          pushed += out.sent;
          dead.push(...out.invalid);
        }
        if (devices.android.length) {
          const out = await sendFcm(devices.android, payload);
          pushed += out.sent;
          dead.push(...out.invalid);
        }
      }
      // Drop tokens Apple or Google reports dead, so they aren't retried forever.
      if (dead.length) {
