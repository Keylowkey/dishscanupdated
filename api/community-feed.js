  ...
  try {
    ...
    if (access_token) {
      try {
        ...
        if (who.ok) { const u = await who.json(); callerId = u && u.id; }
      } catch (e) { /* anonymous is fine */ }
    }

    // 1) Posts (newest first, limit 50)
    const postsRes = await fetch(
      `${SUPABASE_URL}/rest/v1/posts?select=*&order=created_at.desc&limit=50`,
      { headers }
    );
    if (!postsRes.ok) {
    ...
    if (!Array.isArray(posts) || posts.length === 0) {
      return res.status(200).json({ posts: [], reactions: [], callerId });
    }
