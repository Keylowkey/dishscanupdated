...
export default async function handler(req, res) {
  ...
  try {
    ...
    let existingMeta = {};
    try {
      const getRes = await fetch(`${SUPABASE_URL}/auth/v1/admin/users/${uid}`, {
        headers: { 'apikey': SERVICE_KEY, 'Authorization': `Bearer ${SERVICE_KEY}` }
      });
      if (getRes.ok) {
        const u = await getRes.json();
        existingMeta = (u && u.user_metadata) || {};
      }
    } catch (e) { /* proceed with just the new metadata */ }
    const user = { id: uid, user_metadata: existingMeta };
